...
class AndOperator(OperatorBase):
    ...
    def eval(self, *args):
        left, right = args
        return ParticleCollection([atom for atom in left if atom in right])
...
class StringAttributeSelection(SelectionBase):
    ...
    def eval(self, atoms, args):
        return ParticleCollection(
            [atom for atom in atoms if getattr(atom, self.token) in args]
        )
