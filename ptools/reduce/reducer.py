...
class Reducer:
    ...
    def reduce(
        self,
        ignore_exceptions: ExceptionTypeContainer | None = None,
        warn_exceptions: ExceptionTypeContainer | None = None,
    ) -> None:
...
def reduce(
    ...
    warn_exceptions = warn_exceptions or []
    ignore_exceptions = ignore_exceptions or all_exceptions()

    if forcefield not in FORCEFIELDS:
        raise ValueError(
            f"Force field {forcefield!r} is not supported "
